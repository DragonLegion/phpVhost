<?php
class Vhost {
	/* unset all global variables if they are set */
	...
	/*
	 *This section calls the connection functions and connects to the server specified in the config file
	...
	private function _txtsql() {
		global $_CONF;
		$this->txtSQL = new txtSQL($_CONF['sql']['host']);
		$this->txtSQL->strict(0);
		$this->txtSQL->connect($_CONF['sql']['user'],$_CONF['sql']['pass']);
		$this->txtSQL->select_db($_CONF['sql']['db']);
		$this->db = TRUE;
	}
	/*
	...
	private function _sNotice($snotice) {
		...
		if (stripos($snotice, "(o)") !== FALSE) {
			//operup notices (or check for (O) or (o) in case-sensitive)
